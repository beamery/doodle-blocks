...
Blockly.BlockSvg = function(workspace, prototypeName, opt_id) {
   ...
   * @protected
   */
  this.svgGroup = Blockly.utils.createSvgElement('g', {}, null);
  /** @type {SVGElement} */
 ...
 * @protected
 */
Blockly.BlockSvg.prototype.isGlowingBlock = false;
...
Blockly.BlockSvg.prototype.initSvg = function() {
  ...
  if (!this.workspace.options.readOnly && !this.eventsInit_) {
    Blockly.bindEventWithChecks(this.getSvgRoot(), 'mousedown', this,
                       this.onMouseDown_);
    var thisBlock = this;
    Blockly.bindEvent(this.getSvgRoot(), 'touchstart', null,
                       function(e) {Blockly.longStart(e, thisBlock);});
  }
...
Blockly.BlockSvg.prototype.unselect = function() {
  ...
  var event = new Blockly.Events.Ui(null, 'selected', this.id, undefined);
  event.workspaceId = this.workspace.id;
  Blockly.Events.fire(event);
...
Blockly.BlockSvg.prototype.setGlowBlock = function(isGlowingBlock) {
  this.isGlowingBlock = isGlowingBlock;
  this.updateColour();
};
...
Blockly.BlockSvg.terminateDrag = function() {
  if (Blockly.BlockSvg.onMouseUpWrapper_) {
    Blockly.unbindEvent(Blockly.BlockSvg.onMouseUpWrapper_);
    Blockly.BlockSvg.onMouseUpWrapper_ = null;
  }
  if (Blockly.BlockSvg.onMouseMoveWrapper_) {
    Blockly.unbindEvent(Blockly.BlockSvg.onMouseMoveWrapper_);
    Blockly.BlockSvg.onMouseMoveWrapper_ = null;
  }
  ...
  if (Blockly.dragMode == Blockly.DRAG_FREE) {
    ...
    if (selected) {
      if (Blockly.replacementMarker) {
        Blockly.BlockSvg.removeReplacementMarker();
      } else if (Blockly.insertionMarker) {
        Blockly.Events.disable();
        if (Blockly.insertionMarkerConnection) {
...
Blockly.BlockSvg.prototype.setParent = function(newParent) {
  if (newParent == this.parentBlock) {
    return;
  }
  var svgRoot = this.getSvgRoot();
  if (this.parentBlock && svgRoot) {
...
Blockly.BlockSvg.prototype.moveBy = function(dx, dy) {
  goog.asserts.assert(!this.parentBlock, 'Block has parent.');
...
Blockly.BlockSvg.prototype.onMouseDown_ = function(e) {
  ...
  if (this.isInFlyout) {
    ...
    if (e.type == 'touchstart' && Blockly.utils.isRightButton(e)) {
      Blockly.Flyout.blockRightClick(e, this);
      e.stopPropagation();
      e.preventDefault();
    }
    return;
...
Blockly.BlockSvg.prototype.onMouseUp_ = function(e) {
  ...
  if (Blockly.selected && Blockly.highlightedConnection &&
      deleteArea != Blockly.DELETE_AREA_TOOLBOX) {
    // Connect two blocks together.
    Blockly.localConnection.connect(Blockly.highlightedConnection);
    if (this.rendered) {
      // Trigger a connection animation.
      // Determine which connection is inferior (lower in the source stack).
      var inferiorConnection = Blockly.localConnection.isSuperior() ?
          Blockly.highlightedConnection : Blockly.localConnection;
...
Blockly.BlockSvg.prototype.moveConnections_ = function(dx, dy) {
  ...
  for (i = 0; i < this.childBlocks.length; i++) {
    this.childBlocks[i].moveConnections_(dx, dy);
  }
};
...
Blockly.BlockSvg.prototype.setDragging_ = function(adding) {
  if (adding) {
    ...
    Blockly.draggingConnections =
        Blockly.draggingConnections.concat(this.getConnections(true));
  ...
  for (var i = 0; i < this.childBlocks.length; i++) {
    this.childBlocks[i].setDragging_(adding);
  }
};
...
Blockly.BlockSvg.prototype.handleDragFree_ = function(oldXY, newXY, e) {
  var dxy = goog.math.Coordinate.difference(oldXY, /** @type {!goog.math.Coordinate} */ (this.dragStartXY_));
  var group = this.getSvgRoot();
  if (this.useDragSurface_) {
  ...
  if (Blockly.localConnection && Blockly.highlightedConnection) {
...
Blockly.BlockSvg.prototype.updatePreviews = function(closestConnection,
  ...
  if (Blockly.highlightedConnection &&
    ...
    if (Blockly.replacementMarker) {
      Blockly.BlockSvg.removeReplacementMarker();
    } else if (Blockly.insertionMarker && Blockly.insertionMarkerConnection) {
      Blockly.BlockSvg.disconnectInsertionMarker();
    }
...
Blockly.BlockSvg.removeReplacementMarker = function() {
  ...
  if (Blockly.highlightedConnection.type == Blockly.INPUT_VALUE &&
    !Blockly.highlightedConnection.isConnected()) {
    (/** @type {!Blockly.BlockSvg} */ (Blockly.replacementMarker)).highlightShapeForInput(
        Blockly.highlightedConnection, false);
  } else {
    Blockly.replacementMarker.highlightForReplacement(false);
  }
...
Blockly.BlockSvg.prototype.connectInsertionMarker_ = function(localConnection,
  ...
  if (!Blockly.insertionMarker) {
    ...
    if (insertingBlock.mutationToDom) {
      ...
      Blockly.insertionMarker.domToMutation(oldMutationDom);
    }
    Blockly.insertionMarker.setInsertionMarker(true, insertingBlock.width);
    Blockly.insertionMarker.initSvg();
  }
  ...
  if (insertionMarkerConnection != Blockly.insertionMarkerConnection) {
    ...
    if (insertionMarkerConnection.type == Blockly.PREVIOUS_STATEMENT &&
      ...
      Blockly.bumpedConnection = closestConnection.targetConnection;
    }
    // Renders insertion marker.
    insertionMarkerConnection.connect(closestConnection);
    Blockly.insertionMarkerConnection = insertionMarkerConnection;
  }
};
...
Blockly.BlockSvg.disconnectInsertionMarker = function() {
  ...
  if (Blockly.insertionMarkerConnection ==
      ...
      !Blockly.insertionMarker.previousConnection.targetConnection)) {
  ...
  // Inside of a C-block, first statement connection.
    ...
    Blockly.insertionMarker.unplug(true);
    if (previousBlockNextConnection) {
      previousBlockNextConnection.connect(innerConnection);
    }
  }
  else {
    Blockly.insertionMarker.unplug(true /* healStack */);
  }

  if (Blockly.insertionMarkerConnection.targetConnection) {
  ...
  Blockly.insertionMarker.getSvgRoot().setAttribute('visibility', 'hidden');
};
...
Blockly.BlockSvg.prototype.setInsertionMarker = function(insertionMarker, opt_minWidth) {
  ...
  this.insertionMarkerMinWidth_ = opt_minWidth ? opt_minWidth : 0;
  this.updateColour();
};
 ...
 */
Blockly.BlockSvg.prototype.dispose = function(healStack, opt_animate) {
  if (!this.workspace) {
  ...
  goog.dom.removeNode(this.svgGroup);
...
Blockly.BlockSvg.prototype.disposeUiEffect = function() {
  ...
  var xy = this.workspace.getSvgXY(/** @type {!Element} */ (this.svgGroup));
  // Deeply clone the current block.
  var clone = this.svgGroup.cloneNode(true);
...
Blockly.BlockSvg.prototype.setHighlighted = function(highlighted) {
  ...
  if (highlighted) {
    this.svgPath_.setAttribute(
        'filter', 'url(#' + this.workspace.options.embossFilterId + ')');
  } else {
 ...
 * @param {number|string|undefined} colourSecondary Secondary HSV hue value, or #RRGGBB
 *    string.
 * @param {number|string|undefined} colourTertiary Tertiary HSV hue value, or #RRGGBB
 *    string.
 */
 ...
 * @protected
 */
Blockly.BlockSvg.prototype.appendInput = function(type, name) {
  var input = Blockly.BlockSvg.superClass_.appendInput.call(this, type, name);
